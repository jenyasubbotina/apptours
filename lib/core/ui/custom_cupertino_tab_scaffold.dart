...
///
/// {@tool snippet}
/// This samples shows how [CustomCupertinoTabController] can be used to switch tabs in
...
///  * [RestorableCustomCupertinoTabController], which is a restorable version
///    of this controller.
class CustomCupertinoTabController extends ChangeNotifier {
  /// Creates a [CustomCupertinoTabController] to control the tab index of [CupertinoTabScaffold]
  /// and [CupertinoTabBar].
  ///
  ...
  /// [CupertinoTabScaffold] controlled by this [CustomCupertinoTabController], as well
...
class CupertinoTabScaffold extends StatefulWidget {
  ...
  /// [RestorableCustomCupertinoTabController].
  ///
...
class _CupertinoTabScaffoldState extends State<CupertinoTabScaffold> with RestorationMixin {
  RestorableCustomCupertinoTabController? _internalController;

  CustomCupertinoTabController get _controller => widget.controller ?? _internalController!.value;
  ...
  void _updateTabController([CustomCupertinoTabController? oldWidgetController]) {
    if (widget.controller == null && _internalController == null) {
      // No widget-provided controller: create an internal controller.
      _internalController = RestorableCustomCupertinoTabController(initialIndex: widget.tabBar.currentIndex);
      if (!restorePending) {
    ...
    return DecoratedBox(
      ...
      child: Stack(
        children: <Widget>[
          ...
          content,
          MediaQuery(
            data: existingMediaQuery.copyWith(textScaler: const TextScaler.linear(1)),
            child: Align(
...
///
/// The [CustomCupertinoTabController] is accessible via the [value] getter. During
/// state restoration, the property will restore [CustomCupertinoTabController.index]
/// to the value it had when the restoration data it is getting restored from
/// was collected.
class RestorableCustomCupertinoTabController extends RestorableChangeNotifier<CustomCupertinoTabController> {
  /// Creates a [RestorableCustomCupertinoTabController] to control the tab index of
  /// [CupertinoTabScaffold] and [CupertinoTabBar].
  ///
  /// The `initialIndex` must not be null and defaults to 0. The value must be
  /// greater than or equal to 0, and less than the total number of tabs.
  RestorableCustomCupertinoTabController({int initialIndex = 0})
      : assert(initialIndex >= 0),
  ...
  @override
  CustomCupertinoTabController createDefaultValue() {
    return CustomCupertinoTabController(initialIndex: _initialIndex);
  }

  @override
  CustomCupertinoTabController fromPrimitives(Object? data) {
    assert(data != null);
    return CustomCupertinoTabController(initialIndex: data! as int);
  }
